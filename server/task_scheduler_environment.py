...
class TaskSchedulerEnvironment(Environment):
    """
    Task Scheduler Environment — a real-world productivity simulation.
    An AI agent learns to prioritise and complete tasks before their deadlines.
 
    3 difficulty levels:
      - Easy:   5 tasks, generous deadlines, effort=1 each
      - Medium: 8 tasks, tighter deadlines, effort=1-3
      - Hard:   10 tasks, tight deadlines, new urgent task appears mid-episode
    """
    ...
    def reset(self, difficulty: str = "easy") -> TaskSchedulerObservation:
        ...
        return TaskSchedulerObservation(
            ...
            message=f"Episode started! Difficulty: {difficulty}. "
                    f"Complete {len(self._tasks)} tasks before their deadlines.",
            score=0.0,
        )
    ...
    def step(self, action: TaskSchedulerAction) -> TaskSchedulerObservation:
        ...
        # --- Hard mode: inject urgent task at step 3 ---
        if (self._difficulty == "hard"
                and self._current_step == 3
                and not self._urgent_task_injected):
            urgent = Task(
        ...
        if task_id not in valid_ids:
            ...
            if progress >= task.effort:
                ...
                message += (f"Working on '{task.name}' "
                            f"({progress}/{task.effort} steps done, "
                            f"{steps_left} steps until deadline). ")
 
        # --- Check for missed deadlines ---
        for task in self._tasks:
            if (not task.completed
                    and task.deadline < self._current_step
                    and not getattr(task, '_deadline_missed', False)):
                reward -= 0.3
                task.completed = True
        ...
        # --- Penalise ignoring high priority tasks ---
        for task in self._tasks:
            if (not task.completed
                    and task.priority == "high"
                    and task.task_id != task_id
                    and task.deadline - self._current_step <= 2):
                reward -= 0.1
